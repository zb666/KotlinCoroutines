...
object SoundHelper {
    ...
    //专注星球
    const val FOCUSPLANET_NEW_APPEAR = 29 //新物种出现
    const val FOCUSPLANET_COMMON_TREE = 30 //小树互动
    const val FOCUSPLANET_ENERGY = 31 //能量棒挥动音效
    const val FOCUSPLANET_GET_COIN = 32 //金币收入书包音效

    const val APPREA = 34
    const val CYCLE = 35

    private val soundPool = SoundPool.Builder().setMaxStreams(FOCUSPLANET_NEWELEMENT_DIALOG + 1).build()
    ...
    fun init(context: Context) {
        ...
        //专注星球
        soundPool.load(context, R.raw.focusplanet_newelement_appear, 1)
        soundPool.load(context, R.raw.focusplanet_tree, 1)
        soundPool.load(context, R.raw.foucsplanet_getcoin, 1)
        soundPool.load(context, R.raw.focusplanet_newelement_dialog, 1)

        soundPool.load(context, R.raw.tree_appear, 1)
        soundPool.load(context, R.raw.tree_cycle, 1)
